...
function maakjson(dataset) {
    hulp = {};
    hulp.id = dataset.id;
    hulp.keep = dataset.keep.slice();
    hulp.change = dataset.change.slice();
    hulp.aantalkeep = dataset.aantalkeep;
    hulp.aantalchange = dataset.aantalchange;
    hulp.ww=dataset.wachtwoord;
    return hulp;
...
function create_datasets() {
   var gamesjson={}
    ...
    let rawdata = fs.readFileSync('games/gamesjson.json', 'utf8')
    gamesjson = JSON.parse(rawdata);
    for (var i in gamesjson.games) {
        dataset = Dataset(gamesjson.games[i].id, gamesjson.games[i].ww);
        dataset.keep = gamesjson.games[i].keep.slice();
        dataset.change = gamesjson.games[i].change.slice();
        dataset.aantalkeep = gamesjson.games[i].aantalkeep;
        dataset.aantalchange = gamesjson.games[i].aantalchange;
        dataset.lasttime= gamesjson.games[i].lasttime;
        console.log('dataset   ',dataset.id ," toegevoegd")
    }
...
function save_datasets() {
    ...
    for (i in DATASET_LIST) {
        if (i != "123446") {
            ...
            gamesjson.games.push(maakjson(dataset));
        }
    }
    console.log(gamesjson.games);
    fs.writeFileSync("games/gamesjson.json", JSON.stringify(gamesjson));
}
create_datasets();
...
app.get('/alles_opslaan',(req,res)=>{
    console.log('alles opslaan')
    save_datasets();
